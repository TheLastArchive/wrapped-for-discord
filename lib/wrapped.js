...
async function handle_wrapped() {
    ...
    let global_data = await get_data(start, end);
    global_object.global_stats = parser.parse_wrapped(global_data, global_object.global_stats);
    for (const user of users) {
        let user_object = parser.create_user_wrapped_object(user.toJSON());  
        const user_data = await get_data(start, end, user.user_id);
        global_object.user_stats.push(parser.parse_wrapped(user_data, user_object));
    }
