...
async function handle_recent_tracks(tracks, user) {
    ...
    tracks.forEach((track) => {
        ...
        history_data_arr.push(parse_history_data(track, ids, user.user_id));
        if (track.played_at > user.played_at) {
            user.played_at = track.played_at;
        }
    });
...
function parse_history_data(track, ids, user_id) {
    ...
    // console.log(typeof history.played_at);
    return history;
}
...
async function bulk_insert(model, data) {
    ...
    // if (model == sequelize.models.History) {
    //     console.log("Before");
    //     console.log(data);
    // }
