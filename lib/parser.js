...
function parse_wrapped(data, obj) {
    data.history.count.forEach(item => {
        let [track, album, artist, total_time_listened] = get_history_data(item.track_id, data);
 ...
 */
function get_history_data(track_id, data) {
    const track = data.tracks.find((track) => track.track_id === track_id).toJSON();
