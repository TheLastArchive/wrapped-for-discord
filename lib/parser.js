...
function parse_wrapped(data, obj) {
    data.history.count.forEach(item => {
        let [track, album, artist, total_time_listened] = get_history_data(item, data);
...
function get_history_data(item, data) {
    const track = data.tracks.find((track) => track.track_id === item.track_id).toJSON();
    const album = data.albums.find((album) => album.album_id === track.album_id).toJSON();
    const artist = data.artists.find((artist) => artist.artist_id === track.artist_id).toJSON();
...
function create_user_object(user) {
    user.tracks = [];
    user.albums = [];
    user.artists = [];
    user.total_count = 0;
